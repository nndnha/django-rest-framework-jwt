...
def jwt_encode_handler(payload):
    key = api_settings.JWT_PRIVATE_KEY or jwt_get_secret_key(
        payload.get('user_id') if api_settings.JWT_GET_USER_SECRET_KEY else None
    )
    return jwt.encode(
        payload,
        key,
...
def jwt_decode_handler(token):
    ...
    key = api_settings.JWT_PUBLIC_KEY or jwt_get_secret_key(
        jwt.decode(token, None, False).get('user_id') if api_settings.JWT_GET_USER_SECRET_KEY else None
    )
    return jwt.decode(
        token,
        key,
